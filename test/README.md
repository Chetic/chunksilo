...
- Test-only dependencies come from the `test` extra in `pyproject.toml` (`pip install -e '.[test]'`)
...
- **Test files are excluded from the release package**: The release workflow only includes production files (`src/chunksilo/`, `requirements.txt`, `config.yaml`, `docs/`, etc.). Test files remain in the repository for development but are not packaged for end users.
